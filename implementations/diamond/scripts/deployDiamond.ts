...
export async function deployDiamond(cut: FacetCut[]): Promise<void> {
  ...
  if (!Array.isArray(cut) || cut.length === 0) {
    throw new Error(
      "Invalid cut parameter. Expected an array of FacetCut objects.",
    );
  }
  ...
  const diamondCutAbi = JSON.parse(
    readFileSync(
      join(
        __dirname,
        "../artifacts/contracts/diamond/interfaces/IDiamondCut.sol/IDiamondCut.json",
      ),
      "utf8",
    ),
  ).abi;
  ...
  //   const fragment = diamondContract.Interface.getFragment("init");
  //   console.log("fragment", fragment);
  const functionCall = diamondContract.interface.encodeFunctionData("init", []);
